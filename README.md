...
```bash
target/packages/daily_git-0.1.5-aarch64-apple-darwin.tar.gz
...
```bash
curl -fsSL https://github.com/RaphaelNY/daily-report-cli/releases/latest/download/daily_git-installer.sh | bash -s -- --prefix "$HOME/.local" --version 0.1.5
```
...
bash ./daily_git-installer.sh \
  --archive ./daily_git-0.1.5-aarch64-apple-darwin.tar.gz \
  --prefix "$HOME/.local"
```
...
daily_git update --version 0.1.5
```
